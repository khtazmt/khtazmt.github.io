...
- # 常用快捷键
	...
	- to全部折叠/展开
	- td切换文档模式（减小缩进；此模式下回车enter为换行而非新增块）
...
- # 插件
	...
	- Move Block：右键菜单、页面标题右侧箭头或快捷键把块引用、块嵌入、块的内容等移动到当天日记等位置，这样通过日志展示更新内容就相对顺手了
...
- # 可能常用的markdown语言
  id:: 65a7b394-7f9b-42a3-b8f7-7a94faf19512
	- 学习
		...
		- 我觉得比较简单的学习方式是学习者直接在（支持markdown语言的）自己日常用的文本编辑软件中的文本（比如自己写过/编辑过的文章，最起码是看过或想看的文章）中应用markdown语言（可以先创建副本再在副本中操作），或者将这样的文本复制/导出到vscode和爱用的编辑器里看看不同软件、插件的复制/导出、粘贴/导入和显示/预览的效果，试着整理一下（比如按logseq的常规用法缩进缩进再缩进），乃至试着去除编辑器的显示/预览和编辑的多余格式（logseq还是有不少的）
		- 学习方式：从语言到效果，从效果到语言
